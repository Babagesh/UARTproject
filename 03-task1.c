...
static void uart_tx_callback(struct UARTDRV_HandleData * handle, Ecode_t transferStatus, uint8_t * data, UARTDRV_Count_t transferCount);
    ...
    if(sl_button_get_state(&sl_button_btn0))
      {
        ...
        if(buffer_index == 0)
          {
            sprintf(temp_buffer, "Buffer is empty");
            UARTDRV_Transmit(uart_handle, temp_buffer, strlen(temp_buffer), uart_tx_callback);
          }
        else
          {
            UARTDRV_Transmit(uart_handle, uart_buffer, buffer_index, uart_tx_callback);
          }
